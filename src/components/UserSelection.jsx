    ...
    return (
        <>
            ...
            <div className="my-8 text-sm flex flex-col  sm:flex-row  gap-8 flex-wrap border-b border-slate-700 sm:items-center pb-4">
                <button className={`flex gap-2 items-center w-fit hover:text-ftsecondary space-x-16 ${activeUser === null ? "text-white border border-slate-700 rounded-full p-2" : ""}`} onClick={() => onSelectUser(null)}>Todos</button>
                {
                    newUsers.map((name) => (
                        <button onClick={() => onSelectUser(name)} className={`flex gap-2 items-center text-sm w-fit hover:text-ftsecondary space-x-16 ${activeUser === name ? "text-white border border-slate-700 rounded-full p-2" : ""}`} key={name}>
